...
import { changeDate, thousandsSystem, getFontSize, random, addDays } from "@helpers/libs";
...
const Accommodation = ({ navigation }) => {
  ...
  const backgroundSelected = (params) =>
    route === params ? (mode === "light" ? light.main5 : dark.main2) : light.main2;

  const textColorSelected = (params) =>
    route === params ? (mode === "light" ? light.textDark : dark.textWhite) : light.textDark;
  ...
  const saveHosted = async ({ data, cleanData }) => {
    ...
    if (reservationSelected.type === "accommodation") {
      ...
      data.id = id;
      const obj = { ...data };
      const end = addDays(new Date(year, month - 1, daySelected), parseInt(data.days - 1));
      obj.end = end.getTime();
  ...
  const Hosted = ({ type }) => {
    ...
    useEffect(() => {
      const standard = standardReservations.flatMap((item) => {
        const { nomenclature, ref } = nomenclatures.find((n) => n.id === item.id);
      ...
      const accommodation = accommodationReservations
        ...
        .flatMap((item) => {
          const { nomenclature, ref } = nomenclatures.find((n) => n.id === item.ref);
          const { name: zone } = zones.find((g) => g.ref === ref);
          let customer = null;
          if (item.owner) {
            // Vemos si tiene afiliacion con cliente
        ...
        const hostedWithSearch = hosted.filter((h) => {
          ...
          if (
            ...
            if (
              ...
              return;
            if (filters.minDays && h.days < parseInt(filters.minDays.replace(/\D/g, "")))
              return;
            if (filters.maxDays && h.days > parseInt(filters.maxDays.replace(/\D/g, "")))
              return;
  ...
  const Location = () => {
    ...
    }, [search, filters, standardReservations, accommodationReservations, everybody]);
          ...
          {location.map((item) => {
            return (
              <View key={item.id} style={{ marginBottom: 10 }}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  <GuestTable hosted={item.hosted} tableOptions={{ group: false }} />
                </ScrollView>
              </View>
            );
          })}
  ...
  const Groups = () => {
    return (
      <View style={{ height: SCREEN_HEIGHT / 1.55 }}>
        <TextStyle bigParagraph color={mode === "light" ? light.textDark : dark.textWhite}>
          GRUPOS
        </TextStyle>
        <FlatList
          data={zones}
          ...
          renderItem={({ item }) => {
                ...
                const value =
                  n.type === "standard"
                    ...
                    : accommodationReservations.reduce((a, b) => {
                        if (b.ref === n.id && !dateValidation(new Date(b.checkIn), currenDate))
                          return a + 1;
                        return a;
                      }, 0);
            ...
            return (
              <TouchableOpacity
                onLongPress={() => {
                  navigation.navigate("ZoneInformation", { zoneID: item.id });
                }}
        ...
        <>
          <View>
            <View style={styles.row}>
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                {route && (
                  <TouchableOpacity onPress={() => setRoute("")} style={{ marginRight: 5 }}>
                    <Ionicons
                      name="arrow-back"
                      size={getFontSize(25)}
                      color={mode === "light" ? light.textDark : dark.textWhite}
                ...
                <ButtonStyle
                  ...
                  backgroundColor={light.main2}
                  onPress={() => {
                    Alert.alert("PROXIMAMENTE", "ESTA VENTANA LA VOY A RECREAR, UN NUEVO DISENO Y MAS RAPIDO")
              ...
              <View style={{ flexDirection: "row" }}>
                <TouchableOpacity
                  onPress={() => {
                    Alert.alert(
                      "PROXIMAMENTE",
                      "ESTA VENTANA LA VOY A RECREAR, UN NUEVO DISENO Y MAS RAPIDO"
                    );
                    // setRoute("historical")
                  }}
                >
                ...
                >
                  ...
                  >
                    <Picker
                      ...
                      onValueChange={(value) => {
                        if (value === "CreateZone") return navigation.navigate("CreateZone");
