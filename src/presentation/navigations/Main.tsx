...
import App from "./App";
import { startServerWatcher, stopServerWatcher } from "infrastructure/api/server.watcher";
...
const Main: React.FC = () => {
  ...
  // Processes config from the server
  useGetConfig();
  ...
  // Server watcher to check server status
  useEffect(() => {
    if (session) startServerWatcher();
    else stopServerWatcher();
  }, [session]);
