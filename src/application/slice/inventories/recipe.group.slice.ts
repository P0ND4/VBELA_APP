...
const recipeGroup = (collection: Partial<Collection>) => collection?.recipeGroup ?? [];

const initialState: Group[] = [];

export const recipeGroupSlice = createSlice({
  name: "recipe-group",
  initialState,
  reducers: {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(changeAll, (_, action) => recipeGroup(action.payload));
  },
});

export const { add, edit, remove, removeByInventoryID, clean, change } = recipeGroupSlice.actions;
export default recipeGroupSlice.reducer;
